 ...
 *
 * Run:  npx tsx scripts/demo-test.ts
 *
...
function makeToken(userId: string, rooms: string[] = []): string {
  return jwt.sign(
...
function connectWs(userId: string, rooms: string[] = [], port = 3001): Promise<WebSocket> {
...
async function main(): Promise<void> {
  ...
  const alice = await connectWs('alice', ['room:general']);
  const welcomeAlice = await waitForMessage(alice);
  ...
  const bob = await connectWs('bob');
  await waitForMessage(bob); // welcome message
  bob.send(JSON.stringify({ type: 'subscribe', room: 'room:chat' }));
  await waitForMessage(bob); // subscribe ack

  // Both Alice and Bob are in room:chat now
  ...
  console.log(`  Alice received Bob's message: ${JSON.stringify(aliceGotBob)}`);
  ...
  const charlie = await connectWs('charlie', [], 3002); // Connect to instance-2
  const welcomeCharlie = await waitForMessage(charlie);
  console.log(`  Charlie connected to instance-2: ${JSON.stringify(welcomeCharlie)}`);
  await sleep(500);

  // Send message from instance-1's REST API to charlie (who is on instance-2)
  ...
  try {
    ...
    console.log('  ⚠ Cross-instance message not received (may need Redis pub/sub tuning)\n');
  }
